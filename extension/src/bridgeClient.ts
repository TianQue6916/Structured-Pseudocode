...
export async function analyzeContent(
  ...
  const timeoutId = setTimeout(() => controller.abort(), 180000); // 120 秒超时（DeepSeek 可能慢）
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try { const err = await response.json(); 
        // 优先取 diagnostics 中的消息，再取 error 字段
        if (err.diagnostics?.[0]?.message) errMsg = err.diagnostics[0].message;
    ...
    if (error instanceof Error) {
      if (error.name === 'AbortError') {
        throw new Error('请求超时（180秒），DeepSeek API 响应过慢，请稍后重试');
      }
...
export async function generateCode(
  ...
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 180000); // 120 秒超时
